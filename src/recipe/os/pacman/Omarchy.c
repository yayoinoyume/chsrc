...
/**
 * omarchycn 项目在 Gitea 上自建的 [omarchy] 仓库镜像，
 * 每 6 小时同步一次官方 stable 通道 (pkgs.omarchy.org)，仅 x86_64
 */
static MirrorSite_t OmarchyCn =
{
  IS_DedicatedMirrorSite,
  "omarchycn", "OmarchyCn", "omarchycn 项目自建镜像站", "https://git.zacharyzhang.com/ZacharyZhang-NY/omarchycn",
  {NotSkip, NA, NA, "https://git.zacharyzhang.com/api/packages/ZacharyZhang-NY/arch/omarchy/x86_64/lmstudio-bin-0.4.23-1-x86_64.pkg.tar.zst", ACCURATE} // 约 1 GB
};

#define OS_Pacman_MirrorList "/etc/pacman.d/mirrorlist"
#define OS_Pacman_Omarchy_Conf "/etc/pacman.conf"

/* Omarchy 官方 stable 镜像，官方模板中默认的 Arch 主仓库源 */
#define OS_Omarchy_Stable_Mirror "https://stable-mirror.omarchy.org"
...
os_omarchy_prepare ()
{
  ...
  chef_set_recipe_last_updated (this, "2026-09-05");
  ...
  /* omarchy 底层即 Arch Linux，主仓库源与 arch 完全一致，直接借用 os_arch 的源列表 */
  chef_use_other_dish_sources (this, &os_arch_dish);
}
 ...
 *   2. https://github.com/basecamp/omarchy 官方 mirrorlist 模板 (仅 x86_64)
 */
void
os_omarchy_setsrc (char *option)
{
  /* Omarchy 官方仅发布 x86_64 仓库，ARM 端点不存在 */
  ...
  /* 官方 stable 镜像 fallback 放在新源之后兜底(目标即官方 stable 镜像时无需重复) */
  char *to_write = xy_strcat (3, "Server = ", source.url, "/$repo/os/$arch\n");
  if (!xy_streql (source.url, OS_Omarchy_Stable_Mirror))
    to_write = xy_strcat (4, to_write, "Server = ", OS_Omarchy_Stable_Mirror, "/$repo/os/$arch\n");

  /* 幂等: 先清掉 mirrorlist 中所有生效的 Server 行(含之前累积), 再写入新源 + 官方 stable 兜底 */
...
os_omarchyopr_prepare ()
{
  ...
  chef_set_note (this,
    "OPR 是 omarchy 官方专属软件包仓库；omarchycn 项目提供其 stable 通道的镜像，使用时需导入其 registry key。可额外使用 chsrc set omarchy 来更换 Arch Linux 源",
    "OPR is omarchy's official dedicated package repository; the omarchycn project mirrors its stable channel, which requires importing its registry key. "
...
/**
 * @consult
 *   1. https://git.zacharyzhang.com/ZacharyZhang-NY/omarchycn 详见其 docs/pacman-repo.md（镜像同步策略与密钥指纹）
 *   2. https://docs.gitea.com/usage/packages/arch （Gitea Arch 仓库接入方式）
 */
void
os_omarchyopr_setsrc (char *option)
{
  ...
  /* 上游默认源与可用镜像均基于 stable 通道，edge 通道用户换源后将切换到 stable */
  bool is_edge = xy_run_get_status ("grep -q '^Server = https://pkgs.omarchy.org/edge/' " OS_Pacman_Omarchy_Conf) == 0;
  if (is_edge && !xy_streql (source.mirror->code, "user"))
    chsrc_warn2 ("检测到当前使用 edge 通道，上游默认源与可用镜像均基于 stable 通道，继续执行将切换到 stable");
